# HEALTH AGENT V11.1 — EXECUTION PLAN

**Based on:** HEALTH_AGENT_V11_1_AUDIT.md  
**Date:** April 3, 2026  
**Duration:** 10 weeks  
...
This execution plan converts the audit findings into a practical, dependency-ordered build plan. Work is organized into 5 phases over 10 weeks, with each phase building on the previous one.

**Critical Path:**
1. DailyHealthSnapshot model (foundation for everything)
2. Missing core engines (Cardiovascular, Metabolic, Sexual Health, Nutrition)
3. Central Recommendation Engine (unifies fragmented logic)
4. UI alignment to V11.1 spec
...
## ✅ Complete / Leave Alone (Do Not Touch)

These components are 85%+ complete and functioning well. Leave them alone unless bugs are found.

| Component | File | Status | Notes |
|-----------|------|--------|-------|
| Recovery Engine | services/recoveryEngineService.ts | 85% | Fully functional, leave as-is |
| Stress Engine | services/stressEngineService.ts | 85% | Fully functional, leave as-is |
| Joint Health Engine | services/jointHealthEngineService.ts | 85% | Fully functional, leave as-is |
| Adherence Engine | services/adherenceEngineService.ts | 85% | Fully functional, leave as-is |
| Recovery Types | types/recoveryEngine.ts | Complete | Well-defined types |
| Stress Types | types/stressEngine.ts | Complete | Well-defined types |
| Joint Health Types | types/jointHealthEngine.ts | Complete | Well-defined types |
| Adherence Types | types/adherenceEngine.ts | Complete | Well-defined types |
| Point-in-Time Service | services/pointInTimeService.ts | Complete | Change tracking works |
| Sleep Number Integration | services/sleepNumberSyncService.ts | 75% | Connection and sync functional |
| Bloodwork Extraction | services/bloodworkExtractionService.ts | 75% | PDF OCR works |
| Bloodwork Normalization | services/bloodworkNormalizationService.ts | 75% | Analyte normalization works |
| Bloodwork Trends | services/bloodworkTrendService.ts | 70% | Trend calculation works |

---

## 🔨 Build Next (Priority Order)

Work items ordered by dependency. Items at the top must be built before items below them.

### PHASE 1: Foundation (Weeks 1-2)

#### 1.1 Create DailyHealthSnapshot Model
**Objective:** Create unified data structure that all engines can share  
**Complexity:** Medium  
**Risk:** High (everything depends on this)  
**Dependencies:** None (this is the foundation)  

**Files to Create:**
- `server/src/types/dailyHealthSnapshot.ts`

**Files to Modify:**
- All engine services to consume/produce DailyHealthSnapshot
...
export interface DailyHealthSnapshot {
  ...
  workout: {
    ...
    readinessStatus: 'low' | 'moderate' | 'high';
    workoutLoad?: number;
  ...
  dataQuality: {
    ...
    deviceSyncRecency: string; // "14 minutes ago"
    lastUpdated: string;
  };
}
```
...
- All engines need to read from and write to this unified structure
- Control Tower needs this to aggregate data
- Recommendation Engine needs this to generate recommendations
- Prediction Engine needs this for forecasting
- UI needs this for display

---

#### 1.2 Build Cardiovascular Risk Engine
**Objective:** Calculate cardiovascular risk score from bloodwork, BP, HR, body fat  
**Complexity:** Large  
**Risk:** Medium  
**Dependencies:** DailyHealthSnapshot model must exist first  

**Files to Create:**
- `server/src/services/cardiovascularEngineService.ts`
- `server/src/types/cardiovascularEngine.ts`
- `server/src/controllers/cardiovascularEngineController.ts`
- `server/src/routes/cardiovascularEngineRoutes.ts`

**Key Functions:**
```typescript
export async function getCardiovascularRiskToday(userId: string): Promise<CardiovascularRecord>
export function calculateCardiovascularRiskScore(inputs: CardiovascularInputs): number
export function assessRiskLevel(score: number): 'low' | 'moderate' | 'high'
export function generateCardiovascularRecommendations(record: CardiovascularRecord): Recommendation[]
```

**Inputs:**
- Bloodwork: LDL, HDL, triglycerides, ApoB, Lp(a), hsCRP
- Vitals: BP (systolic/diastolic), resting HR, HRV
- Body composition: body fat percentage
- Age, gender from baseline

**Outputs:**
- cardiovascularRiskScore (0-100)
- riskLevel (low/moderate/high)
- trend insights
- recommendations

**Algorithm:**
- Weight bloodwork markers by clinical significance
- Factor in BP (systolic >130 or diastolic >80 = elevated risk)
- Factor in resting HR (>70 = elevated risk)
- Factor in body fat (>25% men, >32% women = elevated risk)
- Calculate composite risk score

---

#### 1.3 Build Metabolic Engine
**Objective:** Track glucose, A1c, insulin sensitivity, metabolic risk  
**Complexity:** Large  
**Risk:** Medium  
**Dependencies:** DailyHealthSnapshot model must exist first  
...
- `server/src/types/metabolicEngine.ts`
- `server/src/controllers/metabolicEngineController.ts`
- `server/src/routes/metabolicEngineRoutes.ts`

**Key Functions:**
```typescript
export async function getMetabolicStatusToday(userId: string): Promise<MetabolicRecord>
export function calculateMetabolicScore(inputs: MetabolicInputs): number
export function assessGlucoseRisk(glucose: number, a1c: number): 'low' | 'moderate' | 'high'
export function estimateInsulinSensitivity(inputs: MetabolicInputs): number
export function generateMetabolicRecommendations(record: MetabolicRecord): Recommendation[]
```

**Inputs:**
- Bloodwork: glucose, A1c, insulin (if available), triglycerides
...
- glucoseRiskSignals
- A1cTrend
- insulinSensitivity estimate
- recommendations

**Algorithm:**
- Glucose: <100 optimal, 100-125 prediabetic, >125 diabetic
- A1c: <5.7 optimal, 5.7-6.4 prediabetic, >6.4 diabetic
- Factor in body fat and visceral fat
- Factor in nutrition adherence
- Calculate composite metabolic health score

---

#### 1.4 Build Sexual Health Engine
**Objective:** Aggregate testosterone, libido, erectile function from bloodwork + interview  
**Complexity:** Large  
**Risk:** Medium  
**Dependencies:** DailyHealthSnapshot model must exist first  

**Files to Create:**
- `server/src/services/sexualHealthEngineService.ts`
- `server/src/types/sexualHealthEngine.ts`
- `server/src/controllers/sexualHealthEngineController.ts`
- `server/src/routes/sexualHealthEngineRoutes.ts`

**Key Functions:**
```typescript
export async function getSexualHealthToday(userId: string): Promise<SexualHealthRecord>
export function calculateSexualHealthScore(inputs: SexualHealthInputs): number
export function assessLibidoTrend(inputs: SexualHealthInputs): 'low' | 'moderate' | 'high'
export function assessErectileFunctionScore(inputs: SexualHealthInputs): number
export function generateSexualHealthRecommendations(record: SexualHealthRecord): Recommendation[]
```

**Inputs:**
- Bloodwork: testosterone (total, free), estradiol, SHBG, prolactin
- Interview: libido level (1-10), erectile function (1-10), morning erections (frequency)
...
**Outputs:**
- sexualHealthScore (0-100)
- libidoTrend
- erectileFunctionScore
- hormoneStatus (optimal/suboptimal/low)
- recommendations

**Algorithm:**
- Testosterone: Age-adjusted optimal ranges
- Libido: Self-reported 1-10 scale
- Erectile function: Self-reported 1-10 scale
- Morning erections: Frequency per week
- Factor in sleep quality (poor sleep = lower testosterone)
- Factor in stress (high stress = lower libido)
- Factor in body fat (high body fat = lower testosterone)

---

### PHASE 2: Core Intelligence (Weeks 3-4)

#### 2.1 Build Nutrition Engine
**Objective:** Calculate calorie/macro targets based on goals, body composition, activity  
**Complexity:** Large  
**Risk:** Low  
**Dependencies:** DailyHealthSnapshot, Body Composition data  

**Files to Create:**
- `server/src/services/nutritionEngineService.ts`
- `server/src/types/nutritionEngine.ts`
- `server/src/controllers/nutritionEngineController.ts`
- `server/src/routes/nutritionEngineRoutes.ts`

**Key Functions:**
```typescript
export async function getNutritionTargetsToday(userId: string): Promise<NutritionRecord>
export function calculateTDEE(baseline: BaselineProfile, bodyComp: BodyComposition): number
export function calculateMacroTargets(tdee: number, goals: Goals, bodyComp: BodyComposition): MacroTargets
export function calculateHydrationTarget(weight: number, activityLevel: string): number
export function generateNutritionRecommendations(record: NutritionRecord): Recommendation[]
```

**Inputs:**
- Baseline: age, gender, height, weight, activity level
- Body composition: weight, lean mass, body fat
- Goals: weight goal, body fat goal
- Workout frequency

**Outputs:**
- calorieTargets (maintenance, deficit, surplus)
- macroTargets (protein, carbs, fat in grams)
- hydrationTargets (ounces per day)
- mealTimingRecommendations
- recommendations

**Algorithm:**
- Calculate TDEE using Mifflin-St Jeor + activity multiplier
- Protein: 1g per lb lean mass (minimum)
- Apply calorie deficit/surplus based on goals (-500 for fat loss, +300 for muscle gain)
- Distribute remaining calories between carbs and fats
- Hydration: 0.5-1 oz per lb body weight

---

#### 2.2 Build Prediction Engine
**Objective:** Forecast weight, body fat, A1c, recovery, performance trends  
**Complexity:** Large  
**Risk:** Medium  
**Dependencies:** DailyHealthSnapshot, all core engines, historical data  

**Files to Create:**
- `server/src/services/predictionEngineService.ts`
- `server/src/types/predictionEngine.ts`
- `server/src/controllers/predictionEngineController.ts`
- `server/src/routes/predictionEngineRoutes.ts`

**Key Functions:**
```typescript
export async function getPredictionsToday(userId: string): Promise<PredictionRecord>
export function forecastWeight(history: BodyComposition[], weeks: number): number
export function forecastBodyFat(history: BodyComposition[], weeks: number): number
export function forecastA1c(history: BloodworkResult[], weeks: number): number
export function forecastRecovery(history: RecoveryRecord[], weeks: number): number
export function forecastPerformance(history: WorkoutRecord[], weeks: number): number
```

**Inputs:**
- Historical trends from all engines (minimum 4 weeks of data)
- Current trajectory (improving/stable/declining)
- Adherence rate

**Outputs:**
- projectedWeight (4/8/12 weeks)
- projectedBodyFat (4/8/12 weeks)
- projectedA1c (12/24 weeks)
- projectedRecovery (4 weeks)
- projectedPerformance (4/8/12 weeks)
- confidence intervals

**Algorithm:**
- Use linear regression for short-term forecasts (4-8 weeks)
- Use exponential smoothing for long-term forecasts (12+ weeks)
- Adjust predictions based on adherence rate
- Provide confidence intervals (±X%)

---

#### 2.3 Build Central Recommendation Engine
**Objective:** Unified recommendation lifecycle (generated → accepted → completed)  
**Complexity:** Large  
**Risk:** High (critical for V11.1 spec)  
**Dependencies:** DailyHealthSnapshot, all core engines  

**Files to Create:**
- `server/src/services/recommendationEngineService.ts`
- `server/src/types/recommendationEngine.ts`
- `server/src/controllers/recommendationEngineController.ts`
- `server/src/routes/recommendationEngineRoutes.ts`

**Database Migration:**
- Create `recommendations` table with lifecycle state tracking
...
export async function rejectRecommendation(recommendationId: string, reason: string): Promise<void>
export async function snoozeRecommendation(recommendationId: string, until: Date): Promise<void>
...
export function detectConflicts(recommendations: Recommendation[]): ConflictReport
```

**Lifecycle States:**
- `generated` - Just created by an engine
...
- `important` - Significant impact on goals
- `optimization` - Nice to have, incremental improvement

**Aggregation Logic:**
- Collect recommendations from: Workout, Supplement, Recovery, Stress, Joint, Sexual Health, Nutrition, Cardiovascular, Metabolic engines
- Deduplicate similar recommendations
- Detect conflicts (e.g., "increase workout intensity" vs "reduce workout load")
- Prioritize by severity and impact
- Limit to top 5-7 active recommendations

---

#### 2.4 Refactor Goal Tracking Engine
**Objective:** Progress tracking, velocity to goal, projected timeline  
**Complexity:** Medium  
**Risk:** Low  
**Dependencies:** DailyHealthSnapshot, Prediction Engine  

**Files to Modify:**
- `server/src/services/goalManagementEngine.ts` (refactor existing)

**Files to Create:**
- `server/src/types/goalTracking.ts` (new comprehensive types)

**Key Functions:**
```typescript
export async function trackGoalProgress(userId: string): Promise<GoalProgressReport>
export function calculateVelocityToGoal(current: number, target: number, history: number[]): number
export function projectTimelineToGoal(current: number, target: number, velocity: number): Date
export function assessOnTrackStatus(progress: number, timeElapsed: number, timeRemaining: number): 'on_track' | 'off_track' | 'ahead'
```

**Goal Types to Track:**
- Body composition: weight, body fat, muscle mass
- Metabolic: A1c, glucose
- Vitals: BP, resting HR
- Performance: bench press, squat, pushups
- Circumference: chest, biceps, waist
- Strength: grip strength

**Outputs:**
- goalProgress (%)
- velocityToGoal (units per week)
- projectedTimeline (estimated completion date)
- onTrackStatus (on_track/off_track/ahead)
...
#### 3.1 Refactor Control Tower Service
**Objective:** Add derived intelligence, trust/freshness metadata, collapsible structure  
**Complexity:** Medium  
**Risk:** Medium  
**Dependencies:** DailyHealthSnapshot, all core engines  

**Files to Modify:**
- `server/src/services/controlTowerService.ts`
...
export interface ControlTowerData {
  ...
  derivedIntelligence: {
    ...
    metabolicTrend: 'improving' | 'stable' | 'declining';
  };
  
  // NEW: Trust/Freshness Metadata
  trustMetadata: {
    lastUpdated: string; // "Today 6:42 AM"
    deviceSyncRecency: string; // "14 minutes ago"
  ...
  sections: {
    overallHealth: {
      expanded: boolean;
      domainScores: ComponentScore[];
      derivedIntelligence: any;
    };
    priorityAlerts: {
      expanded: boolean;
      alerts: Alert[];
...
- **Fatigue Risk:** Low recovery + high stress + high workout load = high fatigue risk
- **Overtraining Risk:** Declining HRV trend + declining performance + persistent soreness = high overtraining risk
- **Injury Risk:** High joint pain + low recovery + high workout load = high injury risk
- **Sleep Trend:** 7-day moving average of sleep hours
- **Performance Trend:** 7-day moving average of workout performance
- **Metabolic Trend:** Glucose and A1c trend over time

---

#### 3.2 Build Control Tower UI
**Objective:** Collapsible sections, priority alerts, recommendation quick actions  
**Complexity:** Large  
**Risk:** Medium  
**Dependencies:** Refactored Control Tower Service  

**Files to Create:**
- `mobile/src/screens/ControlTowerScreen.tsx`

**Files to Delete:**
- `mobile/src/screens/HomeScreen.tsx` (replace with ControlTowerScreen)
- `mobile/src/screens/DashboardScreen.tsx` (if exists)
- `mobile/src/screens/DashboardV13Screen.tsx` (if exists)
...
│   • Injury Risk: Low                │
│                                     │
│ ▶ Priority Alerts (collapsed)       │
│                                     │
│ ▼ Recommendations (expanded)        │
│   🔴 Critical: Reduce workout load  │
│      [Accept] [Snooze] [View]      │
│   🟡 Important: Add magnesium       │
│      [Accept] [Snooze] [View]      │
│                                     │
...
- Trust/freshness display at top
- Priority alerts with severity indicators
- Recommendation quick actions (Accept/Snooze/View)
- Persistent floating "Ask Coach" button

---

#### 3.3 Build Conversational AI Coach Interface
**Objective:** ChatGPT-style conversational interface, replace form-based interview  
**Complexity:** Large  
...
- `server/src/services/conversationalInterviewService.ts` (consolidates existing interview services)
...
- `server/src/services/hybridInterviewService.ts`
- `server/src/services/interviewAgentService.ts`
- `server/src/services/voiceInterviewService.ts`
...
│ [Voice] [Text] (toggle)             │
│                                     │
│ Coach: Good morning! How did you   │
│        sleep last night?            │
│                                     │
│ You: Pretty well, about 7 hours    │
│                                     │
│ Coach: Great! How's your recovery  │
│        feeling today?               │
│                                     │
│ You: [typing...]                    │
│                                     │
...
- Quick action buttons for uploads
- Daily interview happens through Coach, not separate screen
- Continuous conversation (not hold-to-record)

**Backend Service:**
- Consolidate all interview logic into single `conversationalInterviewService.ts`
- Use OpenAI API for natural language understanding
- Extract structured data from conversational responses
- Save to DailyLog and update engines

---

#### 3.4 Build Recommendations UI
**Objective:** Full lifecycle management (accept/reject/snooze/modify)  
**Complexity:** Medium  
**Risk:** Low  
**Dependencies:** Central Recommendation Engine  
...
│ │ Your recovery score is low and  │ │
│ │ stress is high. Reduce volume   │ │
│ │ by 20% for next 3 days.         │ │
│ │                                 │ │
...
- Full lifecycle actions (Accept, Reject, Snooze, Modify)
- Status tracking (Pending, Accepted, Completed, Expired)
- Rationale display for each recommendation
- History view (show completed/rejected recommendations)

---

### PHASE 4: Orchestration (Weeks 7-8)

#### 4.1 Implement DailyHealthSnapshot Aggregation Service
**Objective:** Aggregate data from all engines into unified snapshot  
**Complexity:** Medium  
**Risk:** Medium  
**Dependencies:** DailyHealthSnapshot model, all core engines  

**Files to Create:**
- `server/src/services/dailyHealthSnapshotService.ts`

**Key Functions:**
```typescript
export async function generateDailyHealthSnapshot(userId: string): Promise<DailyHealthSnapshot>
...
**Logic:**
- Call all engine services in parallel
- Aggregate results into DailyHealthSnapshot structure
- Calculate derived intelligence (fatigue risk, overtraining risk, etc.)
...
**Objective:** Background monitoring, proactive recommendations, risk detection  
**Complexity:** Large  
**Risk:** Medium  
**Dependencies:** DailyHealthSnapshot, all core engines, Recommendation Engine  

**Files to Create:**
- `server/src/cron/continuousMonitoring.ts`

**Schedule:** Every 15 minutes

**Actions:**
1. Sync device data (Sleep Number, Apple Watch, Oura)
2. Update engine snapshots (if new data available)
3. Detect risk patterns:
   - BP spike (>140/90)
   - HRV drop (>20% from baseline)
   - Poor sleep trend (3+ nights <6 hours)
   - High stress sustained (3+ days >7/10)
   - Joint pain escalation
4. Generate priority alerts
5. Update Control Tower derived intelligence
6. Trigger proactive recommendations

**Risk Detection Logic:**
```typescript
export function detectRiskPatterns(snapshot: DailyHealthSnapshot, history: DailyHealthSnapshot[]): Alert[]
```

**Alert Types:**
- `health_risk` - Immediate health concern (BP spike, HRV drop)
- `performance_risk` - Overtraining, injury risk
- `adherence_risk` - Missed workouts, poor adherence
- `goal_risk` - Off track for goals

---

#### 4.3 Implement Data Confidence Weighting
**Objective:** Weight engine calculations by input confidence  
**Complexity:** Medium  
**Risk:** Low  
**Dependencies:** All core engines  

**Files to Modify:**
- All engine services to accept and use confidence weights

**Confidence Levels:**
- **High (1.0):** Device data (Sleep Number, Apple Watch, Oura), bloodwork
- **Medium (0.7):** Interview responses, manual input
- **Lower (0.5):** Derived estimates, inferred values
...
export interface ConfidenceWeightedInput {
  ...
  source: 'device' | 'interview' | 'manual' | 'derived';
}

export function calculateWeightedScore(inputs: ConfidenceWeightedInput[]): number {
  const totalWeight = inputs.reduce((sum, input) => sum + input.confidence, 0);
  const weightedSum = inputs.reduce((sum, input) => sum + (input.value * input.confidence), 0);
...
- All engine score calculations

---

#### 4.4 Implement Time Horizon Intelligence
**Objective:** Short/medium/long-term intelligence layers  
**Complexity:** Medium  
**Risk:** Low  
**Dependencies:** DailyHealthSnapshot, all core engines  
...
**Short-term (Today):**
- Workout adjustments based on readiness
- Recovery status
- Stress level
- Joint pain modifications

**Medium-term (Weekly):**
- Training cycle changes (deload week, intensity week)
...
- Goal progress and velocity
- Performance trends

**Implementation:**
```typescript
export interface TimeHorizonIntelligence {
  shortTerm: {
    todayWorkoutAdjustment: string;
    todayRecoveryStatus: string;
    todayStressLevel: string;
  };
  mediumTerm: {
    weeklyTrainingCycleChange: string;
    weeklySupplementAdjustment: string;
    weeklyNutritionAdjustment: string;
  };
  longTerm: {
    monthlyBodyCompTrend: string;
    monthlyA1cTrend: string;
    monthlyCardiovascularRiskTrend: string;
    monthlyGoalProgress: string;
  };
}
```
...
#### 5.1 Consolidate Duplicate Routes
**Objective:** Remove duplicate route files  
**Complexity:** Small  
**Risk:** Low  
**Dependencies:** None  

**Files to Delete:**
- `server/src/routes/bloodwork.routes.ts` (keep bloodworkRoutes.ts)
- `server/src/routes/bodyComposition.routes.ts` (keep bodyCompositionRoutes.ts)
- `server/src/routes/dailyLogs.ts` (keep dailyLogRoutes.ts)
- `server/src/routes/tapeMeasurements.routes.ts` (keep tapeMeasurementRoutes.ts)
- `server/src/routes/strength.routes.ts` (keep strengthTrackingRoutes.ts)

**Files to Modify:**
- `server/src/index.ts` (update route imports)

---

#### 5.2 Consolidate Duplicate Screens
**Objective:** Remove duplicate mobile screens  
**Complexity:** Small  
**Risk:** Low  
**Dependencies:** Control Tower UI, Coach UI must be built first  

**Files to Delete:**
- `mobile/src/screens/DashboardScreen.tsx` (replaced by ControlTowerScreen.tsx)
- `mobile/src/screens/DashboardV13Screen.tsx` (replaced by ControlTowerScreen.tsx)
- `mobile/src/screens/SupplementUploadScreenNew.tsx` (keep SupplementUploadScreen.tsx)
- `mobile/src/screens/RecoveryScreen.tsx` (keep RecoveryDashboardScreen.tsx)
- `mobile/src/screens/RecoveryStatusScreen.tsx` (consolidate into RecoveryDashboardScreen.tsx)
- `mobile/src/screens/GoalsScreen.tsx` (keep GoalManagementScreen.tsx)
- `mobile/src/screens/BaselineSummaryScreen.tsx` (consolidate into BaselineProfileScreen.tsx)

---

#### 5.3 Consolidate Bloodwork Services
**Objective:** Consolidate overlapping bloodwork processing services  
**Complexity:** Medium  
**Risk:** Low  
**Dependencies:** None  

**Files to Keep:**
- `server/src/services/bloodworkExtractionService.ts` (PDF OCR + extraction)
- `server/src/services/bloodworkNormalizationService.ts` (analyte normalization)
- `server/src/services/bloodworkTrendService.ts` (trend calculation)

**Files to Consolidate/Delete:**
- `server/src/services/bloodworkProcessingService.ts` (merge into extraction)
- `server/src/services/bloodworkAIParser.ts` (merge into extraction)
- `server/src/services/bloodworkPatternMatching.ts` (merge into extraction)

**Files to Replace:**
- `server/src/services/bloodworkAnalysisEngine.ts` (currently stub, replace with real implementation)

**New Implementation for bloodworkAnalysisEngine.ts:**
- Interpret bloodwork results (flag out-of-range values)
- Generate health insights from bloodwork
- Identify concerning trends
- Generate bloodwork-based recommendations
- Feed into Cardiovascular, Metabolic, Sexual Health engines

---

#### 5.4 Update Navigation to 7-Tab Structure
**Objective:** Align mobile navigation to V11.1 spec  
**Complexity:** Small  
**Risk:** Low  
**Dependencies:** Control Tower UI, Coach UI, Recommendations UI must be built first  

**Files to Modify:**
- `mobile/src/navigation/BottomTabNavigator.tsx`

**New Tab Structure:**
1. **Home** → ControlTowerScreen.tsx
2. **Coach** → CoachScreen.tsx
3. **Recommendations** → RecommendationsScreen.tsx
4. **Workout** → WorkoutScreen.tsx (create if doesn't exist)
5. **Trends** → TrendsScreen.tsx (existing)
6. **Data** → HealthDataHubScreen.tsx (existing)
7. **Settings** → SettingsScreen.tsx (existing)
...
## 🔧 Refactor Next (After Foundation)

These items should be refactored AFTER DailyHealthSnapshot and Recommendation Engine are built.

### Refactor Control Tower Service (Week 5)
**Why Wait:** Needs DailyHealthSnapshot and all core engines to be complete first  
**Dependencies:** DailyHealthSnapshot, Cardiovascular, Metabolic, Sexual Health, Nutrition engines

### Refactor Workout Today Service (Week 6)
**Why Wait:** Needs 12-week cycle integration  
**Dependencies:** Workout baseline with cycle tracking

### Refactor Supplement Engine (Week 6)
**Why Wait:** Needs dosage/timing optimization logic  
**Dependencies:** Recommendation Engine for lifecycle management

### Refactor Goal Management Engine (Week 4)
**Why Wait:** Needs Prediction Engine for velocity calculations  
**Dependencies:** DailyHealthSnapshot, Prediction Engine

---

## 🗑️ Delete Later (After Replacements Built)

Do NOT delete these until their replacements are fully built and tested.

### Interview Services (Delete in Week 6)
**Delete After:** CoachScreen.tsx and conversationalInterviewService.ts are built
- hybridInterviewService.ts
- interviewAgentService.ts
- voiceInterviewService.ts
- dynamicFollowUpService.ts

### Interview Screens (Delete in Week 6)
**Delete After:** CoachScreen.tsx is built
- AgentInterviewScreen.tsx
- HybridInterviewScreen.tsx
- DynamicInterviewScreen.tsx

### Dashboard Screens (Delete in Week 6)
**Delete After:** ControlTowerScreen.tsx is built
- HomeScreen.tsx
- DashboardScreen.tsx
- DashboardV13Screen.tsx

### Bloodwork Services (Delete in Week 9)
**Delete After:** Consolidation is complete
- bloodworkProcessingService.ts
- bloodworkAIParser.ts
- bloodworkPatternMatching.ts

### Bloodwork Analysis Stub (Replace in Week 9)
**Replace After:** Real implementation is built
- bloodworkAnalysisEngine.ts (current stub)
...
- ✅ Create DailyHealthSnapshot model
- ✅ Build Cardiovascular Risk Engine
- 🎯 Milestone: DailyHealthSnapshot model complete and Cardiovascular Engine functional

## Week 2: Foundation Part 2
- ✅ Build Metabolic Engine
- ✅ Build Sexual Health Engine
- 🎯 Milestone: All 3 missing health engines complete

## Week 3: Core Intelligence Part 1
- ✅ Build Nutrition Engine
- ✅ Build Prediction Engine
- 🎯 Milestone: Nutrition and Prediction engines functional

## Week 4: Core Intelligence Part 2
- ✅ Build Central Recommendation Engine
- ✅ Refactor Goal Tracking Engine
- 🎯 Milestone: Recommendation lifecycle management complete

## Week 5: UX Alignment Part 1
- ✅ Refactor Control Tower Service
- ✅ Build Control Tower UI
- 🎯 Milestone: Control Tower matches V11.1 spec

## Week 6: UX Alignment Part 2
- ✅ Build Conversational AI Coach Interface
- ✅ Build Recommendations UI
- 🎯 Milestone: Conversational Coach and Recommendations UI complete

## Week 7: Orchestration Part 1
- ✅ Implement DailyHealthSnapshot Aggregation Service
- ✅ Implement Continuous Monitoring
- 🎯 Milestone: Background monitoring and proactive recommendations working

## Week 8: Orchestration Part 2
- ✅ Implement Data Confidence Weighting
- ✅ Implement Time Horizon Intelligence
- 🎯 Milestone: Confidence weighting and time horizon layers complete

## Week 9: Cleanup Part 1
- ✅ Consolidate Duplicate Routes
- ✅ Consolidate Duplicate Screens
- 🎯 Milestone: All duplicate files removed

## Week 10: Cleanup Part 2
- ✅ Consolidate Bloodwork Services
- ✅ Update Navigation to 7-Tab Structure
- 🎯 Milestone: Full V11.1 spec alignment achieved
...
### 1. DailyHealthSnapshot Model (Week 1)
**Risk:** Everything depends on this  
**Mitigation:**
- Design carefully with input from all engine requirements
- Get structure right the first time
- Plan for extensibility (easy to add new fields)
- Test with all engines before proceeding

### 2. Central Recommendation Engine (Week 4)
**Risk:** Complex lifecycle management, critical for V11.1 spec  
**Mitigation:**
- Start with simple lifecycle (generated → accepted → completed)
- Add complexity incrementally (snooze, modify, expire)
- Test lifecycle state transitions thoroughly
- Plan database schema carefully

### 3. Conversational AI Coach (Week 6)
**Risk:** Major UX change, OpenAI API dependency  
**Mitigation:**
- Build fallback to form-based interview if API fails
- Test natural language understanding thoroughly
- Ensure structured data extraction is accurate
- Gradual rollout (beta test with small group first)

### 4. Continuous Monitoring (Week 7)
**Risk:** Background job failures, performance impact  
**Mitigation:**
- Start with longer intervals (30 minutes), optimize to 15 minutes
...
```
DailyHealthSnapshot (Week 1)
    ├── Cardiovascular Engine (Week 1)
    ├── Metabolic Engine (Week 2)
    ├── Sexual Health Engine (Week 2)
    ├── Nutrition Engine (Week 3)
    ├── Prediction Engine (Week 3)
    │   └── Goal Tracking Engine (Week 4)
    ├── Recommendation Engine (Week 4)
    │   ├── Control Tower Service Refactor (Week 5)
    │   │   └── Control Tower UI (Week 5)
    │   ├── Conversational Coach (Week 6)
    │   └── Recommendations UI (Week 6)
    ├── DailyHealthSnapshot Aggregation (Week 7)
    │   ├── Continuous Monitoring (Week 7)
    │   ├── Data Confidence Weighting (Week 8)
    │   └── Time Horizon Intelligence (Week 8)
    └── Cleanup (Weeks 9-10)
```
...
## Medium (3-5 days)
- DailyHealthSnapshot model
- DailyHealthSnapshot aggregation service
- Control Tower service refactor
- Goal tracking engine refactor
...
# POSTPONE UNTIL AFTER FOUNDATION

These items should NOT be started until DailyHealthSnapshot and Recommendation Engine are complete:

1. **Control Tower UI** - Needs refactored Control Tower Service
2. **Conversational Coach** - Needs DailyHealthSnapshot for context
3. **Recommendations UI** - Needs Central Recommendation Engine
4. **Continuous Monitoring** - Needs all engines and DailyHealthSnapshot
5. **Data Confidence Weighting** - Needs all engines to be refactored
6. **Time Horizon Intelligence** - Needs all engines and historical data
7. **Cleanup/Consolidation** - Wait until replacements are built and tested
...
- ✅ DailyHealthSnapshot model exists and is used by all engines
- ✅ Cardiovascular, Metabolic, Sexual Health engines functional
- ✅ All engines read from and write to DailyHealthSnapshot
- ✅ API endpoints for new engines working

## Phase 2 Success (Week 4)
- ✅ Nutrition Engine calculates accurate targets
- ✅ Prediction Engine forecasts with confidence intervals
- ✅ Central Recommendation Engine manages lifecycle
- ✅ Goal Tracking Engine shows progress and velocity

## Phase 3 Success (Week 6)
- ✅ Control Tower UI matches V11.1 spec
- ✅ Conversational Coach replaces form-based interview
- ✅ Recommendations UI supports full lifecycle
- ✅ Users can accept/reject/snooze recommendations

## Phase 4 Success (Week 8)
- ✅ Continuous monitoring runs every 15 minutes
- ✅ Risk patterns detected and alerts generated
- ✅ Data confidence weighting applied to all calculations
- ✅ Time horizon intelligence layers working

## Phase 5 Success (Week 10)
- ✅ All duplicate files removed
- ✅ Navigation matches 7-tab structure
- ✅ Bloodwork services consolidated
- ✅ Full V11.1 spec alignment achieved

---

# FINAL NOTES

1. **Do NOT skip DailyHealthSnapshot** - Everything depends on this foundation
2. **Do NOT delete files until replacements are tested** - Avoid breaking existing functionality
3. **Test each phase before moving to next** - Ensure stability at each milestone
4. **Prioritize engine completeness over UI polish** - Get the intelligence right first
5. **Keep existing working engines untouched** - Recovery, Stress, Joint, Adherence are solid
6. **Document as you build** - Update types, add comments, maintain clarity
7. **Monitor performance** - Continuous monitoring and aggregation services must be efficient
8. **Plan for rollback** - Keep old screens/services until new ones are proven stable

Recovery Engine Status — COMPLETE

Recovery AI enrichment is now validated end-to-end through RecommendationEngine persistence and retrieval. AI-generated Recovery recommendations are successfully normalized, validated, persisted, and retrieved, with observable differentiation from fallback output.

Next Phase
Stress Engine AI Migration

Phase 3 — AI Enrichment Migration
...
Conflict Resolution
Supersession Logic
AI vs Fallback Differentiation

Validation Scripts

validate:recovery:ai-success
validate:recovery:ai-fallback
validate:recovery:e2e

Completed Milestones

Add:

Phase 3 Milestone — AI Engine Pattern Proven

Completed:

• Recovery Engine AI Migration
• Stress Engine AI Migration

Result:

AI Engine architecture validated and reusable.
...
Joint Engine AI Migration

This helps keep your execution roadmap accurate.

AI Engine Milestone Log
Milestone 1 — Recovery AI Engine

Status: Complete
Architecture: Validated

Milestone 2 — Stress AI Engine
...
This gives you a running architecture history — extremely valuable later.
...
Goal-Driven Optimization is implemented and validated. Control Tower v6 optimizes plans based on user goals, balancing multiple objectives and dynamically adjusting strategies. The Personal AI Health Agent now operates as a goal-driven autonomous optimization system.
...
Control Tower Daily Intelligence API

You are implementing Control Tower Daily Intelligence API for the Personal AI Health Agent.

This service sits on top of the Daily AI Plan Aggregator and prepares:

Production-Ready Home Screen Intelligence

OBJECTIVE

Create:

Control Tower Daily Intelligence Service
...
data completeness
device sync recency
UI-ready card structure
summary confidence
STEP 1 — Create Types

Create:

src/types/controlTowerDaily.ts

Define:

export interface ControlTowerFreshness {
  lastUpdated: string
  dataFreshness: 'fresh' | 'stale' | 'partial'
  missingSources?: string[]
}

export interface ControlTowerConfidence {
...
export interface ControlTowerDaily {
  userId: string
  date: string
  summary: any
  cards: ControlTowerCard[]
  freshness: ControlTowerFreshness
  confidence: ControlTowerConfidence
...
src/services/controlTowerDailyService.ts

Main Function:

getControlTowerDaily(userId: string)
...
Build UI cards
Return Control Tower object
STEP 3 — Load Daily AI Plan

Call:

dailyAIPlanService.getDailyAIPlan(userId)
...
Workout last updated
Nutrition last updated
Device sync timestamps

Rules:

All updated today → fresh
...
Recovery available +10
Workout available +10
Nutrition available +10
Predictive available +10
Goals available +10
...
STEP 7 — Example Cards

Summary

"Today calls for a controlled execution plan"
...
Emerging recovery decline

STEP 8 — Controller

Create:

src/controllers/controlTowerDailyController.ts
STEP 9 — Routes

Create:

src/routes/controlTowerDailyRoutes.ts

Endpoints:

GET /control-tower/:userId/today
GET /control-tower/:userId/history
STEP 10 — Logging

Add logs:

control_tower_loaded
control_tower_cards_built
control_tower_confidence_calculated

STEP 11 — Validation Script

Create:

validate-control-tower-e2e.ts
...
Example Output
{
  summary: "Controlled execution day",
  cards: [
    Recovery Card
    Workout Card
    Nutrition Card
...
Control Tower object generated
Cards built
...
Use Daily AI Plan Aggregator as input.

Build lightweight aggregation layer.
