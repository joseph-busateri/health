-- Wave 1, Step 2: Workout Baseline Document Engine Schema
-- Create tables for workout document management and structured workout baselines

-- Enable UUID extension if not already enabled
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- workout_documents table
-- Stores uploaded workout document metadata
...
-- Stores structured workout context extracted from documents
...
-- Stores raw extracted text segments from documents
...
-- Tracks changes to workout baselines for future agent refinement
...
COMMENT ON TABLE public.workout_documents IS 'Stores uploaded workout document metadata and file references';
COMMENT ON TABLE public.workout_baselines IS 'Stores structured workout context extracted from documents';
COMMENT ON TABLE public.workout_extracted_sections IS 'Stores raw text segments extracted from workout documents';
COMMENT ON TABLE public.workout_change_log IS 'Tracks changes to workout baselines for audit and refinement';
COMMENT ON VIEW public.latest_workout_documents IS 'Convenient view to get the most recent workout document per user';
COMMENT ON VIEW public.latest_workout_baselines IS 'Convenient view to get the most recent workout baseline per user';
