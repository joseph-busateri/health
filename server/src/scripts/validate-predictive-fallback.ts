...
const TEST_USER_ID = 'predictive-fallback-test';

async function main() {
  console.log('='.repeat(80));
  console.log('Predictive Intelligence Fallback Validation');
  console.log('='.repeat(80));
  ...
  const predictiveAIEnabled = process.env.USE_AI_ENRICHMENT_PREDICTIVE === 'true';

  console.log(`USE_AI_ENRICHMENT: ${process.env.USE_AI_ENRICHMENT}`);
  console.log(`USE_AI_ENRICHMENT_PREDICTIVE: ${process.env.USE_AI_ENRICHMENT_PREDICTIVE}`);

  if (aiEnabled && predictiveAIEnabled) {
    console.log('\n⚠️  WARNING: AI enrichment is enabled. For true fallback validation, set USE_AI_ENRICHMENT_PREDICTIVE=false');
  ...
  if (predictiveResponse?.data?.recommendation) {
    const rec = predictiveResponse.data.recommendation;

    if (rec.source === 'fallback' || !rec.source) {
      console.log(`✅ Recommendation source is fallback: ${rec.source || 'undefined'}`);
    } else if (rec.source === 'ai_enriched' && !predictiveAIEnabled) {
  ...
  if (predictiveResponse?.success === true) {
  ...
  if (predictiveResponse?.data?.riskLevel) {
    console.log(`✅ Risk level present: ${predictiveResponse.data.riskLevel}`);
  } else {
    errors.push('Risk level missing');
    console.log('❌ Risk level missing');
  }
  ...
  const outputPath = path.join(outputDir, 'predictive-fallback.json');
  fs.writeFileSync(
    outputPath,
    JSON.stringify(
      {
