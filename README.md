...
**2026-04-18**: Moved Cardiovascular Risk to dedicated section on HOME screen for prominence. Changes: (1) Removed Cardiovascular Risk from Quick Actions section. (2) Created dedicated "Cardiovascular Risk" section on ModernHomeScreen, positioned immediately after hero card. (3) Section uses same QuickActionCard component with heart-pulse icon and red theme (#EF4444). (4) Section title: "Cardiovascular Risk", subtitle: "10-year CVD risk assessment". (5) Navigation handler: `navigation.navigate('ActuarialRisk')`. Status: **PRODUCTION-READY**. Cardiovascular Risk now has its own prominent section on the home screen, making it immediately visible upon app launch. Navigation flow: Home Tab → ModernHome → Cardiovascular Risk section (first section after hero) → ActuarialRiskScreen. This provides better organization and visibility for this critical health metric.
