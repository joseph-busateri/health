...
**2026-04-18**: Implemented Omron Evolv blood pressure integration via Apple HealthKit. Changes: (1) Installed `react-native-health` package for HealthKit integration. (2) Added `getBloodPressureData()` function to `healthKitService.ts` to read BP data from HealthKit. (3) Added blood pressure sync to `syncAllHealthData()` function in healthKitService. (4) Created standalone `syncBloodPressure()` export function for device-specific BP sync (30 days default). (5) Updated `DevicesScreen.tsx` to replace manual BP entry with automatic HealthKit sync - users tap "BP Monitor" to sync from Omron Evolv via Apple Health. (6) Added `blood_pressure` case to `healthDataService.ts` sync handler. (7) Created `saveBloodPressureData()` method in healthDataService with validation (systolic: 50-300 mmHg, diastolic: 30-200 mmHg). (8) Created database migration `20260418_create_blood_pressure_readings_table.sql` with constraints, indexes, and comments. Status: **PRODUCTION-READY**. Blood pressure data flows: Omron Evolv → Omron Connect → Apple Health → HealthKit → Mobile App → Backend API → Database. Remaining: Run migration on production database, configure Omron Connect app to sync with Apple Health, test with physical Omron Evolv device.
