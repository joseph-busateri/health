...
**2026-04-18**: Fixed bloodwork AI parser schema mismatch with flexible validation. Changes: (1) Added cleanup function in bloodworkAIParser.ts to convert empty objects {} to null for malformed AI output fields (abnormal_flag, reference_range, reference_range_low/high, unit). (2) Updated BloodworkParseSchema to use 'any' type with custom validators for flexible fields, allowing objects that will be cleaned up. (3) Added 'any' to ValidationSchema type to support flexible validation scenarios. Status: **FIXED**. AI parser now handles malformed output by cleaning up empty objects before validation, preventing fallback to mock data. Previous issue: AI was returning empty objects {} instead of null for missing values, causing type mismatch errors.
