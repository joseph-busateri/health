...
**2026-04-18**: Fixed bloodwork AI parser schema mismatch. Changes: (1) Updated BloodworkParseSchema in aiOutputValidator.ts to include panels array, panel field on markers, reference_range_low/high fields, and extended abnormal_flag enum to include "Normal". (2) Updated AI prompt in bloodworkAIParser.ts to be more explicit about abnormal_flag format (must be string or null, not object). (3) Updated AIParseResult interface to include 'fallback', 'pattern', and 'mock' source types. Status: **FIXED**. AI parser now correctly validates against the expected schema, preventing type mismatch errors that caused fallback to mock data. Previous issue: AI was returning objects for abnormal_flag and including fields not in schema, causing validation failures.
