...
**2026-04-18**: Added 30-second timeout to trend generation to prevent hanging. Changes: (1) Added Promise.race with 30-second timeout to getBloodworkTrendsByUser call in bloodworkProcessingService.ts. (2) Timeout error is treated as insufficient data and skipped, allowing pipeline to continue. Status: **FIXED**. Trend generation no longer hangs indefinitely when database query is slow or stuck. Previous issue: Second document upload would hang at "Starting trend generation" and never complete.
